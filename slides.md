...
# Шорткати для написання промпта
...
| Клавіша | Умова |
|---|---|
| `\` + `Enter` | Працює у всіх терміналах |
...
<!--
Шорткати значно пришвидшують роботу. Esc Esc — найкорисніший: повертає до попереднього промпту і дає його відредагувати, Claude перепише відповідь з нуля. Щодо Ctrl+Enter — його немає в офіційному списку multiline-опцій. Надійніші: \+Enter (працює завжди), Shift+Enter (треба /terminal-setup для VS Code/Alacritty/Zed/Warp), Option+Enter на macOS, Ctrl+J як універсальний line feed. Зверніть увагу на нюанс macOS: для Alt/Option-шорткатів потрібно в терміналі увімкнути "Option as Meta" (iTerm2 → Profiles → Keys, Terminal.app → Profiles → Keyboard). Натискання ? у сесії показує актуальний список шорткатів для вашого оточення.
-->
...
# Shell — aliases, completions, функції

<v-clicks>

### Aliases (у `~/.zshrc` / `~/.bashrc`)

```bash
alias cc="claude"
alias ccp="claude -p"             # print mode
alias ccc="claude -c"             # continue last session
alias ccn="claude -n"             # named session
```

### Tab-completions

```bash
claude --shell-completions zsh > ~/.zsh/completions/_claude
claude --shell-completions bash > /etc/bash_completion.d/claude
claude --shell-completions fish > ~/.config/fish/completions/claude.fish
```

### Корисна функція

```bash
# Швидко запитати Claude з будь-якого місця, не виходячи з pipeline
ask() { claude -p "$*"; }

# Приклад: echo "$ERROR_LOG" | ask "що тут зламалось"
```

</v-clicks>

<!--
Claude Code швидший, коли руки не треба знімати з клавіатури. Aliases зекономлять вам тисячі натискань за місяць. Shell-completions — це не тільки команди, а й прапори та значення: --model sonnet, --effort high підкажуться з Tab. Функція ask — моя улюблена. Бере stdin або аргументи, прокидає в claude -p, повертає відповідь у pipeline. Можна грепати логи, передавати в ask, і одразу отримати діагноз. Тримаєте у dotfiles — у команди швидко поширюється.
-->
...
### Voice dictation — `hold Space → говори → release`

Корисно для довгих промптів і коли руки в іншому процесі.
...
<!--
Claude Code часто працює хвилинами на складних задачах. Замість того щоб дивитися на екран — дайте йому вас ткнути. Базова опція — notifications.mode: system у settings, і термінал буде слати native нотифікацію, коли потрібен дозвіл або задача завершилась. Якщо хочете більше контролю — Stop hook з terminal-notifier (macOS) або notify-send (Linux). Voice dictation — окремий трюк: тримаєте Space, говорите промпт, відпускаєте. Швидше за друк для довгих повідомлень. Це три дрібнички, які роблять день комфортнішим.
-->
...
<!--
Коли ваш setup розростається — агенти, скіли, нетривіальні settings — ви хочете, щоб новачок у команді не починав з нуля. Два рівні шарингу. Проектний рівень — все що в .claude/ всередині репо — автоматично комітиться і всі отримують. Персональний рівень — ~/.claude/ — треба керувати через dotfiles менеджер. chezmoi найпопулярніший, але yadm або stow теж підходять. Команда може тримати shared-dotfiles репо з базовими агентами на кшталт code-reviewer, migration-reviewer, які хочеться мати всім, але персональні налаштування кожен допилює локально.
-->
...
<!--
Visual Explainer — це про виходи, які можна показати людині. Коли ревʼюверу треба зрозуміти тиждень роботи — generate-visual-plan або diff-review зробить інтерактивну сторінку з діаграмами, before/after, edge cases. Project-recap — моя улюблена: відновлює mental-model великого проєкту після відпустки. Fact-check — окрема перевіряє, чи документ ще відповідає кодобазі, і коригує розходження. Все у HTML, без залежностей — шлеш колезі, відкриває у браузері, розуміє за хвилини замість годин.
-->
