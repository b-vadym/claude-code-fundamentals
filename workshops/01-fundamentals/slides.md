...
- **Просунуті можливості** — Claude in Chrome (браузерний агент)
...
| План | Ціна | Ліміти | Моделі | Для кого |
|---|---:|---:|---|---|
| **Pro** | $20/міс ($17 річно) | ~40–45 / 5 год | Sonnet 4.6 | Ознайомлення, особиста робота |
| **Max 5x** | $100/міс | ~225 / 5 год | Sonnet 4.6 + Opus 4.7 | Full-day coding, один розробник |
| **Max 20x** | $200/міс | ~900 / 5 год | Sonnet + Opus + пріоритет | Heavy agents, subagents, паралель |
...
| **API key** (pay-as-you-go) | Haiku $1/$5 · Sonnet $3/$15 · Opus $5/$25 за 1M tok | usage-based | усі | CI/CD, batch, непередбачуваний usage |

**Усі платні плани** включають також **claude.ai (web)** і **Claude Desktop** — той самий акаунт, та сама підписка.

**API knobs**: prompt cache hit = **10%** input price · Batch API = **−50%** на input+output.

</v-clicks>

<DocRef url="https://platform.claude.com/docs/en/about-claude/pricing" label="platform.claude.com — API pricing" :offset="1" />
<DocRef url="https://claude.com/pricing" label="claude.com/pricing" />

<!--
Коротко про ціни, бо це перше питання після "круто, беремо". Pro за двадцятку — це рівно $20 на місяць, або $17 якщо платити річно наперед. На Pro реально можна працювати, але ліміт близько сорока промптів за п'ятигодинне вікно — тобто одна інтенсивна сесія в день максимум. Max 5x за сотню — це вже full-day coding, близько 225 промптів за вікно, плюс доступ до Opus 4.7 для важких задач. Max 20x за двісті — це вже для тих, хто ганяє subagents паралельно і робить агентні pipeline, 900 промптів за вікно. Team — від $25 до $125 за місце, це B2B підписка з SSO, центральним білінгом, адмінкою. Enterprise — seat плюс usage по API-ставках, HIPAA, RBAC, може йти через Bedrock або Vertex. Окремий варіант — просто API key pay-as-you-go: Haiku долар вхід п'ять вихід, Sonnet три на п'ятнадцять, Opus п'ять на двадцять п'ять за мільйон токенів. Нема ліміту на п'ятигодинне вікно, але треба стежити за usage — легко спалити сотні доларів на агентних workflows. Два знижкових knobs для API: prompt cache hit коштує десять відсотків від звичайного input price — якщо повторюєш system prompt між запитами, економиш дев'яносто відсотків на кеш-хітах. Batch API — мінус п'ятдесят відсотків на input і output, але асинхронно, для не-realtime задач типу класифікації. На наступному слайді — що вибрали ми.
-->
...
<!--
Навігаційні шорткати — найчастіше використовувані. Tab / → приймає autocomplete для @, / і prompt suggestions. ↑/↓ ходить по історії промптів поточної сесії. Ctrl+R — reverse search по історії, як у bash: починаєш писати і він знаходить останню команду, що містить ці літери. Ctrl+C — класичне переривання: скасовує поточний ввід або зупиняє генерацію. Esc Esc — найцінніший: повертає до попереднього промпту і дає його відредагувати, Claude перепише відповідь з нуля. Замість видаляти невдалу відповідь — просто Esc Esc і уточни.
-->
...
# Шорткати — редагування тексту

<v-clicks>

**Редагування поточного вводу**
...
> 💡 **macOS:** увімкніть "Option as Meta" у налаштуваннях терміналу, щоб працювали `Alt+B` / `Alt+F` (iTerm2 → Profiles → Keys, Terminal.app → Profiles → Keyboard).
...
<!--
Шорткати для редагування — як у readline/bash. Ctrl+U і Ctrl+K — видалити до початку / до кінця рядка, зручно щоб швидко переписати промпт. Ctrl+Y — вставити назад видалене (yank). Alt+B і Alt+F — переміщення курсором на слово назад і вперед, швидше ніж стрілками по літерах. Ctrl+L — очистити поточний ввід, але історія діалогу залишається. Ctrl+G — найбільш недооцінений: відкриває поточний промпт у зовнішньому $EDITOR (vim, nano, code), після збереження підставляє назад. Для довгих промптів — незамінно. На macOS для Alt-шорткатів потрібно ввімкнути "Option as Meta" у налаштуваннях терміналу. Натискання `?` у сесії показує актуальний список шорткатів для вашого оточення.
-->
...
<!--
Конфігурація sandbox дозволяє тонко налаштувати межі. allowWrite — додаткові директорії для запису. denyRead — заборонити читання чутливих файлів. Network — whitelist доменів. Коли sandbox увімкнений, Bash-команди всередині меж виконуються автоматично — менше промптів, така ж безпека. Далі подивимось на увімкнення і режими.
-->
...
| Платформа | Setup |
|---|---|
| **macOS** | Seatbelt built-in — нічого не треба |
| **Ubuntu/Debian** | `sudo apt-get install bubblewrap socat` |
| **Fedora** | `sudo dnf install bubblewrap socat` |
| **WSL2** | як Linux (WSL1 не підтримується) |
| **Native Windows** | поки не підтримується |

`/sandbox` → відкриває меню вибору режиму.
...
**Auto-allow mode**:
- Bash-команди в sandbox запускаються **без підтвердження**
- Команди, які не можуть бути sandbox-нуті (напр. потребують non-allowed host) — fallback на normal permissions flow
- `rm`/`rmdir` на `/`, `$HOME`, critical paths — завжди prompt

**Regular permissions mode**:
- Усі команди через standard permission flow, навіть sandboxed
- Більше контролю, більше approvals

Файлова/мережна ізоляція ідентична в обох — різниця тільки у auto-approve.

</div>
</div>

<DocRef url="https://code.claude.com/docs/en/sandboxing#enable-sandboxing" label="code.claude.com/docs — Enable sandboxing" />

<!--
На macOS sandbox готовий з коробки через Seatbelt — нічого доставляти. На Linux і WSL2 треба поставити bubblewrap і socat через пакетний менеджер. WSL1 не підтримується — bubblewrap вимагає kernel-фіч доступних тільки у WSL2. Native Windows — ще в планах. Запуск — /sandbox відкриває меню вибору режиму. Auto-allow — мій дефолт на щоденній роботі: sandbox сам проганяє Bash-команди без підтвердження, fallback на normal flow для випадків які не вкладаються у межі. Regular mode — коли треба побачити кожен execution (аудит, paranoia для чутливого коду). В обох режимах fs/network обмеження одинакові.
-->
...
# Sandbox для PHP/Laravel — приклад

```json {all|3-8|9-11|12-19|20-22|all}
{
  "sandbox": {
    "enabled": true,
    "filesystem": {
      ...
      "denyRead": ["~/.ssh", "./.env", "./.env.production"]
    },
    "network": {
      "allowedDomains": [
        "packagist.org",
...
<DocRef url="https://code.claude.com/docs/en/sandboxing#configure-sandboxing" label="code.claude.com/docs — Configure sandboxing" />

<!--
Реальний конфіг для Laravel-проєкту. Читати блоки зверху вниз. Filesystem: за замовчуванням Bash пише лише у working directory, тому явно дозволяємо storage (логи, кеш, сесії, queue), bootstrap/cache (компільовані config і routes), public/build (Vite asset output), ~/.composer (composer кешує пакети глобально), /tmp (phpunit temp-файли, composer temp). denyRead — заборона читати SSH-ключі і .env з виробничими секретами (навіть локально — щоб випадково не потрапили в лог). Network: whitelist доменів для composer і github. Без них composer install попросить дозвіл на кожен домен. excludedCommands: docker несумісний з sandbox (per docs), тому docker-команди виконуються поза sandbox — з обов'язковим promptом. Symfony — той самий принцип, тільки allowWrite має бути ./var замість ./storage+./bootstrap/cache.
-->
...
# Sandbox — escape hatches та gotchas

<v-clicks>

**Несумісні з sandbox інструменти** (per docs):
- `docker` → додавай у `excludedCommands: ["docker *"]`
- `watchman` → для Jest використовуй `--no-watchman`
- Команди що шукають unix-сокети — обережно з `allowUnixSockets`

**Escape hatch — `dangerouslyDisableSandbox`**:
- Коли команда падає через sandbox, Claude може retry її поза sandbox (через normal permissions flow)
- Вимкнути глобально: `"allowUnsandboxedCommands": false`

**Settings merging** — `allowWrite`/`denyRead` зливаються з усіх scopes (managed + user + project + local). Не перезаписують.

**Filesystem permission escalation**:
- Не дозволяй запис у директорії з виконуваними файлами на `$PATH`
- Не дозволяй запис у `.bashrc`/`.zshrc` та configs shells
- Інакше sandbox-ed процес може дати собі execute outside

</v-clicks>

<DocRef url="https://code.claude.com/docs/en/sandboxing#security-limitations" label="code.claude.com/docs — Security limitations" />

<!--
Важливі gotchas, про які мовчать туторіали. docker не запускається у sandbox — per docs прямо. Додавай його в excludedCommands і сприймай як non-sandboxed. watchman (для Jest) теж ламається — для PHPUnit це не проблема, але для frontend (npm run test) додай --no-watchman. Escape hatch: якщо Claude виявив sandbox-incompatible команду, він може retry її поза sandbox-ом через normal permissions flow. Це security trade-off — користувач бачить prompt, але sandbox не блокує. Вимкни через allowUnsandboxedCommands: false якщо параноя. Settings merging — мій улюблений корисний нюанс: можна мати managed корпоративний allowWrite, а project додати свої — злиються. Privilege escalation: якщо дозволив запис у директорію на $PATH — sandbox фактично розірвали, бо sandbox-ed процес може дописати executable. Будь обережний з bin-директоріями.
-->
...
# Мета-промптинг: делегуй Claude'у сам prompting

<v-clicks>

**Ідея**: замість "що написати у промпт" — скажи Claude'у **який результат потрібен**, він сам сформулює промпт/skill/CLAUDE.md.

Три рівні застосування:

| Рівень | Інструмент | Коли |
|--------|-----------|------|
| **Iterative** | Звичайна розмова: "перепиши цей prompt краще" | Ad-hoc — у сесії |
| **Console tool** | Anthropic Console → Prompt Improver | Один раз, перед deploy у production |
| **Automated** | `UserPromptSubmit` hook з rewrite-логікою | На кожен prompt, в Claude Code |

</v-clicks>

<DocRef url="https://platform.claude.com/docs/en/build-with-claude/prompt-engineering/prompting-tools" label="platform.claude.com/docs — Prompting tools" />

<!--
Мета-промптинг — термін для підходу "не пиши промпт сам, попроси Claude написати промпт". Звучить як циркулярна залежність, але працює: Claude знає власну архітектуру краще за тебе і формулює під себе точніше. Три рівні. Перший — iterative: ти у сесії кажеш "ось мій vague prompt, перепиши його як для production". Це безкоштовно і миттєво. Другий — Anthropic Console має окремий інструмент Prompt Improver, який перетворює твій draft на production-ready prompt з chain-of-thought, XML tags, examples. Третій — автоматизація через UserPromptSubmit hook: hook перехоплює кожен твій prompt перед тим як Claude його побачить, проганяє через preprocessor, доповнює або blocks.
-->
...
# Console Prompt Improver — зовнішній інструмент

<v-clicks>

Anthropic Console → Prompt Improver:

**4 кроки, які він робить**:
1. **Example identification** — виймає приклади з твого template
2. **Initial draft** — створює structured template з XML tags
3. **Chain of thought refinement** — додає reasoning instructions
4. **Example enhancement** — переписує приклади під новий reasoning

**Результат** — prompt з:
- Chain-of-thought instructions
- Clear organization через XML tags
- Standardized example formatting
- Strategic prefills

**Коли юзати**:
- Accuracy > latency (response буде довший, дорожчий)
- Complex tasks з reasoning
- Production prompts які стабільно треба

</v-clicks>

<DocRef url="https://platform.claude.com/docs/en/build-with-claude/prompt-engineering/prompting-tools#prompt-improver" label="platform.claude.com/docs — Prompt improver" />

<!--
Prompt Improver — це кнопка "зроби красиво" у Anthropic Console (не у Claude Code — у web-консолі). Ти вводиш свій draft, описуєш проблеми з поточним output-ом, додаєш приклади ідеального output-у — і отримуєш переписаний prompt. Чотири фази роботи, видно прямо у UI. Результат — prompt на XML tags з chain-of-thought, що зазвичай дає кращу точність. Trade-off: prompt стає довший, reasoning — повільніший, тож cost росте. Використовуй для production promtів які ти стабільно викликаєш. Не використовуй для ad-hoc у Claude Code — там простіше попросити Claude переписати у самій сесії.
-->
...
# `UserPromptSubmit` hook — автоматизація у Claude Code
...
```json
{
  "hooks": {
    "UserPromptSubmit": [{
      "hooks": [{
        "type": "command",
        "command": "~/.claude/hooks/improve-prompt.sh"
      }]
    }]
  }
}
...
`improve-prompt.sh` отримує JSON з `prompt`, `session_id`, `cwd`, `transcript_path`. Може:

<v-clicks>

- **Додати context** — `stdout` з додатковими фактами → приклеюється до prompt
- **Заблокувати** — `{"decision": "block", "reason": "..."}` → user бачить reason
- **Перейменувати session** — `hookSpecificOutput.sessionTitle`
- **Збагатити vague prompts** — перетворити "fix bug" у конкретний з relevant file paths

Use case: pre-flight check — якщо prompt < 10 words і немає file mentions → блок з "Додай контекст: який файл, яка поведінка".

</v-clicks>

<DocRef url="https://code.claude.com/docs/en/hooks" label="code.claude.com/docs — Hooks" />

<!--
Найпотужніший рівень мета-промптингу — UserPromptSubmit hook. Запускається ДО того як Claude побачить prompt. Hook отримує JSON із prompt та metadata. Три варіанти дії: (1) додати контекст — просто вивести текст у stdout, він приклеїться до prompt-у; (2) заблокувати — повернути JSON з decision block, user побачить reason і нічого не витратиться; (3) перейменувати сесію автоматично. Практичний приклад: pre-flight checker, який відловлює vague prompts на кшталт "зроби щось" і блокує з підказкою "додай файл, очікуваний result, edge cases". Це економить токени — поганий prompt не спалює контекст на розвідку. Також можна писати hook який автоматично додає "use plan mode" для складних задач або "refer to CLAUDE.md section X" для конкретних типів запитів. На GitHub є готові реалізації — severity1/claude-code-prompt-improver як приклад.
-->
